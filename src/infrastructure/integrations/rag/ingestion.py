...
class IngestionService:
	...
	def _ingest_text_file(self, path: Path, chunking_strategy: str) -> str:
		try:
			content = self._read_text_utf8(path)
			if not content.strip():
				raise ValueError("Text file is empty")
			markdown = MarkdownFormatter.plain_to_markdown(content)
			return self.add_document(markdown, source=str(path), chunking_strategy=chunking_strategy)
		except Exception as e:
			log_error(f"Failed to read text file: {e}")
			raise

	def _ingest_pdf_file(self, path: Path, chunking_strategy: str) -> str:
		try:
			...
			if not pages:
				...
				return self.add_document(ocr_markdown, source=str(path), chunking_strategy=chunking_strategy)
	...
	def _ingest_json_file(self, path: Path, chunking_strategy: str) -> str:
		try:
	...
	def _ingest_csv_file(self, path: Path, chunking_strategy: str) -> str:
		try:
			text = self._read_text_utf8(path)
			if not text.strip():
	...
	def _ingest_html_file(self, path: Path, chunking_strategy: str) -> str:
		try:
			content = self._read_text_utf8(path)
			if not content.strip():
	...
	def _ingest_docx_file(self, path: Path, chunking_strategy: str) -> str:
		try:
