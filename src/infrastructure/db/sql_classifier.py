...
SQL string → SQLClassifier.classify()
    ├── _detect_operation_type()  → SELECT/INSERT/UPDATE/DELETE/DROP/...
    ├── _extract_tables()         → ["user", "thread", ...]
    ├── _has_where_clause()       → bool
    ├── _has_limit_clause()       → bool
    ├── _is_multi_statement()     → bool
    ├── _estimate_rows_affected() → "single"/"multiple"/"all"/"unknown"
    └── _assess_danger_level()    → DangerLevel + warnings
        → SQLClassification dataclass (with auto-determined ApprovalPolicy)
