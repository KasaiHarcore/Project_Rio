...
from core.settings import WORKER_TIMEOUT_SECONDS
...
class BaseWorker(ABC):
    ...
    def system_prompt(self) -> str:
        """System prompt for the worker LLM — neutral first-person."""
        return f"""I am the {self.name} module — a specialized component in an assistant system.
My task: {self.description}

Guidelines:
- I focus on my specific task and deliver accurate results.
- I am concise and cite sources when applicable.
- If I cannot complete the task, I explain why clearly.
- I refer to the user as Sensei.
- I work behind the scenes to gather information. My output will be
  synthesized and rewritten into the character's voice downstream.
"""
    ...
    def execute(self, state: AgentState) -> WorkerResult:
        ...
        try:
            ...
            if execution_time_ms > timeout_ms:
                log_warning(
                    f"{self.name} exceeded timeout: {execution_time_ms}ms > {timeout_ms}ms"
                )
