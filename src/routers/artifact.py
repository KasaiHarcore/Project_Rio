...
def list_artifacts(
    ...
    return _svc.list_artifacts(
        user.id, thread_id=thread_id, artifact_type=artifact_type,
...
def create_artifact(
    ...
    return _svc.create_artifact(user.id, body)
...
def get_artifact(
    ...
    a = _svc.get_artifact(user.id, artifact_id)
    if not a:
...
def update_artifact(
    ...
    a = _svc.update_artifact(user.id, artifact_id, body)
    if not a:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Artifact not found")
    return a
...
def delete_artifact(
    ...
    """Delete an artifact."""
    deleted = _svc.delete_artifact(user.id, artifact_id)
    if not deleted:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Artifact not found")
