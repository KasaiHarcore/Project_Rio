...
class SQLSchemaService:
    ...
    def infer_relevant_tables(self, query: str) -> List[str]:
        ...
        1. **Exact match** — if any real table name (or its singular/plural
           variants, or ``_``-separated parts) appears in the query, include it.
        2. **Synonym map** — a small set of natural-language synonyms that map
           to table names (e.g. "chat" → thread, message).

        If neither pass produces results, **all tables** are returned so the
        LLM always has full context rather than a stale default set.
        ...
        all_tables = snapshot.get_table_names()
        ...
        for table_name in all_tables:
            ...
            # Exact table name in query (e.g. "audit_log")
            if name_lower in query_lower:
                relevant.add(table_name)
                continue

            # Match individual segments (e.g. "audit" or "log" from "audit_log")
            segments = name_lower.split("_")
            for seg in segments:
                if len(seg) >= 3 and seg in query_lower:
                    relevant.add(table_name)
                    break

            # Simple singular/plural heuristic
            if name_lower.endswith("s") and name_lower[:-1] in query_lower:
                relevant.add(table_name)
        ...
        for keyword, tables in synonym_map.items():
        ...
        if relevant:
            ...
            for table_name in list(relevant):
                table_info = snapshot.get_table(table_name)
                if not table_info:
                    continue
                for fk in table_info.foreign_keys:
                    ref = fk.get("referred_table")
                    if ref and ref in all_tables:
