...
import { useActivityMonitor } from './use-activity-monitor'
import { useInterventionStore, InterventionType } from '@/store/intervention-store'
import { useEmotionalStore } from '@/store/emotional-store'
import { useMissionStore } from '@/store/mission-store'
import { generateRioResponse, type ResponseContext } from '@/lib/rio-response-generator'
...
export function useInterventionEngine() {
  ...
  const generateMessage = async (situationType: ResponseContext['situationType'], additionalInfo?: any) => {
    const context: ResponseContext = {
      mood,
      energy: 1.0, // We don't track energy in activityData yet
      affinity,
      relationshipTier,
      streakDays: 0, // Get from emotional store if available
  ...
  useEffect(() => {
    if (preferences.breakFrequency === 0) return // Break reminders disabled
    ...
    // Cooldown: at least 10 minutes between break reminders
    ...
    // 120 min: Aggressive blocking
      ...
      generateMessage('intervention_120min').then((message) => {
        triggerIntervention({
          type: 'blocking',
          message,
          triggerReason: 'long_session_120',
    ...
    if (urgentMissions.length > 0 && !activityData.isIdle) {
      ...
      generateMessage('deadline_pressure', {
        ...
        triggerIntervention({
          type: 'toast',
          message,
          triggerReason: `deadline_pressure_${mission.id}`,
        })
      })
    }
  ...
  useEffect(() => {
    ...
    // More than 24 hours: Welcome back
