...
import { useNoteStore, type Note } from '@/features/notes/store'
import {
...
function CollectionPicker({
    ...
    const handleToggle = (colId: string) => {
        const isActive = currentCollectionIds.includes(colId)
        const next = isActive
            ? currentCollectionIds.filter((id) => id !== colId)
            : [...currentCollectionIds, colId]
        updateNote(noteId, { collectionIds: next })
    }

    const handleClearAll = () => {
        updateNote(noteId, { collectionIds: [] })
        onClose()
    }

    return (
        <div className="absolute right-0 top-full mt-1 z-50 w-48 rounded-xl border shadow-xl py-1 backdrop-blur-xl animate-in fade-in slide-in-from-top-1 duration-150 bg-[#161b22] border-rose-900/30 shadow-black/40">
            <div className="px-3 py-2 flex items-center justify-between border-b border-rose-900/20">
            ...
            {currentCollectionIds.length > 0 && (
                <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-amber-400 hover:bg-amber-500/10 transition-colors text-left"
                >
                    <X size={13} />
                    Remove from all
                </button>
            ...
            {collections.length === 0 ? (
                ...
                collections.map((col) => {
                    const isActive = currentCollectionIds.includes(col.id)
                    return (
                        <button
                            key={col.id}
                            ...
                            className={cn(
                                ...
                                isActive
                                    ? "text-[var(--note-accent)] bg-rose-900/15"
                        ...
                        >
                            ...
                            {isActive && (
                                <span className="ml-auto text-[9px] text-rose-400">active</span>
                            )}
                        </button>
                    )
                })
...
export function NoteList({ onNewNote }: NoteListProps) {
    ...
    const notes = useMemo(() => {
        ...
        if (activeCollectionId) {
            filtered = filtered.filter((n) => (n.collectionIds ?? []).includes(activeCollectionId))
        }
        if (searchQuery.trim()) {
    ...
    }, [allNotes, activeCollectionId, searchQuery])

    const [menuOpenId, setMenuOpenId] = useState<string | null>(null)
    const [collPickerNoteId, setCollPickerNoteId] = useState<string | null>(null)

    const removeNote = useNoteStore((s) => s.removeNote)
    const updateNote = useNoteStore((s) => s.updateNote)

    const getCollectionNames = (colIds?: string[]) => {
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                {loading ? (
                    ...
                    <div className="divide-y divide-[var(--note-divider)]">
                        {/* Column headers */}
                        <div className="grid grid-cols-[1fr_120px_80px_32px] px-4 py-2.5 text-[9px] font-black uppercase tracking-[0.2em] text-[var(--note-muted)]">
                            <span>Note</span>
                        ...
                        {notes.map((note) => {
                            const colName = getCollectionNames(note.collectionIds)
                            return (
                                <div
                                    key={note.id}
                                    onClick={() => setActiveNoteId(note.id)}
                                    className={cn(
                                        "group grid grid-cols-[1fr_120px_80px_32px] px-4 py-3 cursor-pointer transition-all items-center",
                                ...
                                >
                                    ...
                                    <div className="relative" onClick={(e) => e.stopPropagation()}>
                                        {colName ? (
                                            <button
                                                onClick={() => setCollPickerNoteId(
                                                    collPickerNoteId === note.id ? null : note.id
                                                )}
                                        ...
                                        {collPickerNoteId === note.id && (
                                            <CollectionPicker
                                                noteId={note.id}
                                                currentCollectionIds={note.collectionIds ?? []}
                                                onClose={() => setCollPickerNoteId(null)}
                                            />
                                        )}
                                    </div>
