...
import { useNoteStore, type NoteBlock } from '@/features/notes/store'
import {
...
export function NoteSidePanel() {
    ...
    const headings = React.useMemo(() => {
        ...
        // comments like "# ..." inside ```python``` are not treated as outlines.
        const stripped = activeNote.content.replace(/^```[\s\S]*?^```/gm, '')
        const matches = stripped.match(/^#{1,6}\s+(.*)$/gm)
