...
import { useSidebarStore, type WorkspaceFile, SIDEBAR_LIMITS } from '@/features/chat/store'
import { useUIStore, AgentMode } from '@/shared/store/ui-store'
import { chunkFile } from '@/shared/lib/code-chunker'
import {
...
const FILE_TYPE_ICONS: Record<WorkspaceFile['type'], React.ComponentType<Record<string, unknown>>> = {
  code: FileCode2,
  document: FileText,
  data: Table2,
  image: Image,
...
const SOURCE_ICONS: Record<string, React.ComponentType<Record<string, unknown>>> = {
  rag: FileText,
  web: Globe,
  sql: Database,
  file: File,
...
function ModeEmptyIcon({ mode }: { mode: AgentMode }) {
  const icons: Record<AgentMode, React.ComponentType<Record<string, unknown>>> = {
