...
export function MissionBoard() {
  ...
  const themeColor = isPlana ? 'text-rose-500' : 'text-blue-500'
  const bgColor = isPlana ? 'bg-rose-50' : 'bg-blue-50'
  const borderColor = isPlana ? 'border-rose-200' : 'border-blue-200'
  const gradient = isPlana ? 'from-rose-500 to-rose-600' : 'from-blue-500 to-blue-600'
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8 relative z-10 w-full max-w-7xl mx-auto">
      ...
      <div className="mb-12 relative flex items-start justify-between">
        <div>
            <div className={cn("absolute -left-4 -top-4 w-20 h-20 border-l-2 border-t-2 rounded-tl-3xl pointer-events-none transition-colors", borderColor)}></div>
            <h1 className="text-4xl font-black text-slate-800 tracking-tight">
            SCHALE <span className={cn("transition-colors", themeColor)}>OFFICE</span>
            </h1>
            <div className="flex items-center gap-4 mt-2 text-slate-500 font-medium">
            <span className={cn("flex items-center gap-2 text-xs font-bold tracking-widest uppercase px-2 py-1 rounded transition-colors", bgColor, isPlana ? "text-rose-600" : "text-blue-600" )}>
                SYS.VER.3.0
            </span>
            <span className="text-sm">Welcome back, Sensei. There are {MOCK_MISSIONS.filter(m => m.status === 'ACTIVE').length} active tasks pending.</span>
            </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        ...
        {/* System Load */}
        <div className={cn("rounded-2xl border bg-white/60 p-6 backdrop-blur-sm relative overflow-hidden transition-colors", isPlana ? "border-rose-100" : "border-blue-100")}>
            <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">System Load</span>
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
            </div>
            <div className="text-3xl font-black text-slate-700">12<span className="text-sm font-bold text-slate-400 ml-1">%</span></div>
        ...
        <div className={cn("rounded-2xl border bg-white/60 p-6 backdrop-blur-sm relative overflow-hidden transition-colors", isPlana ? "border-rose-100" : "border-blue-100")}>
            ...
            <div className="text-3xl font-black text-slate-700">2.4<span className="text-sm font-bold text-slate-400 ml-1">K</span></div>
                <div className="mt-4 w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                <div className={cn("h-full w-[45%] transition-colors", isPlana ? "bg-rose-500" : "bg-blue-500")}></div>
            </div>
        </div>
        ...
        <div className={cn("rounded-2xl border bg-white/60 p-6 backdrop-blur-sm relative overflow-hidden transition-colors", isPlana ? "border-rose-100" : "border-blue-100")}>
            ...
            <div className="text-3xl font-black text-slate-700">01<span className="text-sm font-bold text-slate-400 ml-1">/ 03</span></div>
            <div className="flex gap-1 mt-4">
                    <div className={cn("h-1 flex-1 rounded-full transition-colors", isPlana ? "bg-rose-500" : "bg-blue-500")}></div>
        ...
        <div className={cn("rounded-2xl border bg-white/60 p-6 backdrop-blur-sm relative overflow-hidden transition-colors", isPlana ? "border-rose-100" : "border-blue-100")}>
             ...
             <div className="text-3xl font-black text-slate-700">14<span className="text-sm font-bold text-slate-400 ml-1">DAYS</span></div>
      ...
      <h2 className="text-lg font-black text-slate-700 mb-6 flex items-center gap-2">
        <span className={cn("w-1.5 h-6 rounded-full transition-colors", isPlana ? "bg-rose-500" : "bg-blue-500")}></span>
        CURRENT MISSIONS
      </h2>
...
function MissionCard({ mission }: { mission: Mission }) {
    ...
    return (
        ...
        >
             {mission.status === 'ACTIVE' && (
                <div className="absolute top-0 right-0 px-3 py-1 bg-blue-500 text-white text-[10px] font-bold rounded-bl-xl z-20">
                    ACTIVE
                </div>
            )}
                ...
                <div className="space-y-1">
                     <div className="flex gap-2 mb-2">
                        {mission.tags.map(tag => (
                            <span key={tag} className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-slate-100 text-slate-500 border border-slate-200">
                                {tag}
                            </span>
                        ))}
                    </div>
