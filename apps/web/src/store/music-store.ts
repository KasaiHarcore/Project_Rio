...
export const useMusicStore = create<MusicState>((set, get) => ({
  ...
  fetchTracks: async () => {
    try {
      ...
      // Preserve any blob tracks from file picker
      ...
      if (lastId && !get().currentTrack) {
        ...
        if (idx >= 0) {
          set({ currentTrack: merged[idx], currentIndex: idx })
        }
      }
    } catch {
      set({ isLoading: false })
  ...
  nextTrack: () => {
    ...
    if (repeatMode === 'one') {
      // Restart same track — handled by seekTo(0) + play in the hook
    ...
    persist('rio-music-last-track', track.id)
    set({ currentTrack: track, currentIndex: nextIdx, isPlaying: true, progress: 0, currentTime: 0 })
  },
  ...
  previousTrack: () => {
    ...
    persist('rio-music-last-track', track.id)
    set({ currentTrack: track, currentIndex: prevIdx, isPlaying: true, progress: 0, currentTime: 0 })
  },
